...
while (cc)
 ...
 if (exist(fgfilename,'file'))
     ...
     fgdata(ng).eta=fgdata(ng).etamax;
     ...
     title(['Maximum surface elevation fro t < ',num2str(fgdata(ng).t)],'FontSize', 12)
     hold off
     
     fgdata(ng).eta=fgdata(ng).etamin;
     figure(2)
     clf
     hold on
     geo_plot2fg;
     afterframe;
     axis([fginfo(ng).xlow,fginfo(ng).xhi,fginfo(ng).ylow,fginfo(ng).yhi]);
     title(['Minimum surface elevation for t < ',num2str(fgdata(ng).t)],'FontSize', 12)
     hold off
