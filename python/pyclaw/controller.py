...
class Controller(object):
    ...
    def plotclaw(self, datadir="."):
        ...
        if sys.platform in ["cygwin", "win32"]:
            ...
            syscmd = " python %s/pyclaw/plotclaw.py  %s" % (pydir, datadir)
        os.system(syscmd)
    ...
    def run(self):
        ...
        logging.info("Solution %s computed for time t=%f" % (0, self.solutions["n"].t))
        ...
        for t in output_times[1:]:
            ...
            logging.info(
                "Solution %s computed for time t=%f" % (frame, self.solutions["n"].t)
            )
    ...
    def get_data(self, claw_path=None):
        ...
        claw_data.add_attribute("xlower", value=self.solution.dimensions[0].lower)
        claw_data.add_attribute("xupper", value=self.solution.dimensions[0].upper)
        if claw_data.ndim > 1:
            claw_data.add_attribute("ylower", value=self.solution.dimensions[1].lower)
            claw_data.add_attribute("yupper", value=self.solution.dimensions[1].upper)
        if claw_data.ndim > 2:
            claw_data.add_attribute("zlower", value=self.solution.dimensions[2].lower)
            claw_data.add_attribute("zupper", value=self.solution.dimensions[2].upper)

        claw_data.add_attribute("mbc", value=self.solution.mbc)
        claw_data.add_attribute(
            "mthbc_xlower", value=self.solution.dimensions[0].mthbc_lower
        )
        claw_data.add_attribute(
            "mthbc_xupper", value=self.solution.dimensions[0].mthbc_upper
        )
        if claw_data.ndim > 1:
            claw_data.add_attribute(
                "mthbc_ylower", value=self.solution.dimensions[1].mthbc_lower
            )
            claw_data.add_attribute(
                "mthbc_yupper", value=self.solution.dimensions[1].mthbc_upper
            )
        if claw_data.ndim > 2:
            claw_data.add_attribute(
                "mthbc_zlower", value=self.solution.dimensions[2].mthbc_lower
            )
            claw_data.add_attribute(
                "mthbc_zupper", value=self.solution.dimensions[2].mthbc_upper
            )
    ...
    def runxclaw(self, verbose=True):
        ...
        try:
            ...
            raise Exception(
                "== controller: Cannot change to directory xdir = %s" % xdir
            )
            return
