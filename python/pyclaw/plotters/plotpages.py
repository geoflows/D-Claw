...
class PlotPagesData(object):
    ...
    def make_html(self):
        ...
        path_to_html_index = os.path.join(
            os.path.abspath(self.plotdir), self.html_index_fname
        )
...
class HtmlIndex(object):
    ...
    def close(self):
        self.file.write("\n</body></html>")
        self.file.close()
        path_to_html_index = os.path.join(os.getcwd(), self.fname)
        print_html_pointers(path_to_html_index)
...
def plots2html(plot_pages_data):
    ...
    if ppd.html_eagle:
        ...
        if homelink:
            html.write(
                '<center><a href="%s">Back to %s</a></center>\n' % (homelink, homelink)
            )
    ...
    html.write(
        "\n<p><h3><a href=%s>Return to Plot Index</a> </h3>" % ppd.html_index_fname
    )
    html.write("\n</center></body></html>\n")
    html.close()
...
def timeframes2html(plot_pages_data):
    ...
    for frameno in framenos:
        ...
        html.write("\n <tr><td>Frame %s, t = %s:</td>" % (frameno, frametimef[frameno]))
        for figno in fignos:
        ...
        if numfigs > 1:
            html.write('\n<td><a href="%s">All figures</a></td>' % allfigsfile[frameno])
        html.write("</tr>\n")
    html.write("</table>\n")
    html.write("</body></html>")
    ...
    if numfigs > 1:
        for iframe in range(numframes):
            ...
            html.write('<html><meta http-equiv="expires" content="0">')
            html.write("<title>Plots</title>")
            html.write("<body>\n<center><h3>All Figures -- Frame %s" % framenos[iframe])
            ...
            if iframe == 0:
                html.write("&#060; &nbsp; &nbsp; ")
                html.write('\n<a href="%s">Index</a> ' % ppd.html_index_fname)
                if numframes > 1:
                    html.write(
                        '&nbsp; &nbsp; <a href="%s"> &#062; </a> '
            ...
            html.write(
                '&nbsp; &nbsp; \n<a href="%s"> ' % allfigsfile[framenos[numframes - 1]]
            )
            html.write("&#062; &#062;</a>  \n")
        ...
        for figno in fignos:
            ...
            if numfigs > 1:
                ...
                for figno2 in fignos:
                    if figno2 == figno:
                        html.write(
                            "\n<font color=red>%s</font>&nbsp;&nbsp;" % fignames[figno]
                        )
                    else:
                        html.write(
                            '\n<a href="%s">%s</a>  &nbsp; &nbsp; '
                ...
                html.write('\n<a href="%s"> All Figures </a>' % allfigsfile[frameno])
            ...
            html.write('\n<a href="%s">  All Frames </a>' % allframesfile[figno])

            html.write(
                "\n<p><h3><a href=%s>Plot Index</a></h3>" % (ppd.html_index_fname)
            )
            if eagle:
                html.write(
                    """<p><h3><a href="../eaglemenu.html">Main Menu for
    ...
    path_to_html_index = os.path.join(
        os.path.abspath(ppd.plotdir), ppd.html_index_fname
    )
    print_html_pointers(path_to_html_index)
...
def htmlmovie(plot_pages_data, pngfile, framenos, figno):
    ...
    text += """
        ...
        """ % (
        plot_pages_data.html_index_fname,
        pngfile[framenos[0], figno],
    )
...
def test(makeplots=True):
    ...
    for n in range(6):
        ...
        if mod(n, 2) == 0:
            pid.latex_preitem = (
                r"""
              \vskip 5pt \noindent{\large\bf Plot of $x^%s$}\vskip 2pt"""
                % n
            )
...
def clawtest():
    ...
    html_index = HtmlIndex(
        fname="vary_mx_index.html", title="Results from running vary_mx.py"
    )
...
def plotclaw2html(plotdata):
    ...
    if numfigs > 1:
        for iframe in range(numframes):
            ...
            if iframe == 0:
                html.write("&#060; &nbsp; &nbsp; ")
                html.write('\n<a href="%s">Index</a> ' % plotdata.html_index_fname)
                if numframes > 1:
                    html.write(
                        '&nbsp; &nbsp; <a href="%s"> &#062; </a> '
                        % allfigsfile[framenos[1]]
    ...
    for iframe in range(numframes):
        ...
        for figno in fignos:
            ...
            html.write(
                "\n<p><h3><a href=%s>Plot Index</a></h3>" % (plotdata.html_index_fname)
            )
            if eagle:
                html.write(
                    """<p><h3><a href="../eaglemenu.html">Main Menu for
    ...
    if numfigs > 1:
        for igauge in range(numgauges):
            ...
            html.write('<html><meta http-equiv="expires" content="0">')
            html.write("<title>Plots</title>")
            html.write("<body>\n<center><h3>All Figures -- Gauge %s" % gaugenos[igauge])
            html.write("<p>\n")
    ...
    for igauge in range(numgauges):
        ...
        for figno in fignos:
            ...
            if numfigs > 1:
                ...
                html.write(
                    '\n<a href="%s"> All Figures </a>' % gauge_allfigsfile[gaugeno]
                )
            ...
            html.write('\n<a href="allgaugesfig%s.html">  All Gauges </a>' % figno)
    ...
    if plotdata.ffmpeg_movie:
        ...
        for figno in fignos_each_frame:
            try:
                ...
                print(
                    (
                        "*** Error creating moviefig%s.mp4 (check that ffmpeg is installed)"
                        % figno
                    )
                )
