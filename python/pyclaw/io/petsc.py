...
def write_petsc(
    solution, frame, path="./", file_prefix="claw", write_aux=False, options={}
):
    r"""
    ...
    if solution.maux > 0 and write_aux:
        write_aux = True
        aux_filename = os.path.join(path, "%s_aux.ptc" % file_prefix) + str(
            frame
        ).zfill(4)
    else:
        write_aux = False
    ...
    if options["format"] == "ascii":
        ...
        if write_aux:
            aux_viewer = PETSc.Viewer().createASCII(
                aux_filename, PETSc.Viewer.Mode.WRITE
            )
...
def read_petsc(
    solution, frame, path="./", file_prefix="claw", read_aux=False, options={}
):
    r"""
