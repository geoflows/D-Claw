...
def integrate(m0, h0, u0, pe0, npoints=100, tend=10.0):
    def rhs(mp, hp, up, pep):
        ...
        f[0] = 2.0 * k * mp * pep / (mu * hp ** 2)
        f[1] = rhorat * 2.0 * k * pep / (mu * hp)
        f[2] = (
            gx
            - max(0.0, gz * rhorat - pep / (rhop * hp))
            * max(0.0, np.tan(phi + np.arctan(mp - meqn)))
            - (1.0 - mp) * 2.0 * mu * up / (rhop * hp ** 2)
        )
        f[3] = -(3.0 * k / (compress * mu * hp ** 2)) * (
            1.0 - 0.5 * compress * rho_f * gz * hp * rhorat
        ) * pep - 3 * up * np.tan(mp - meqn) / (compress * hp)
        # import pdb;pdb.set_trace()
        tt = 5.0
        return (f, meqn)
...
def integrate2(m0, h0, u0, pe0, npoints=100, tend=10.0):
    def fobject(
        q, mn, hn, un, pen, dt, mu, compress, k0, a0, gz, rho_f, rho_s, delta, m_crit
    ):
        ...
        psi3 = -(3.0 * k / (compress * mu * hp ** 2)) * (
            1.0 - 0.5 * compress * rho_f * gz * hp * rhorat
        ) * pep - 3.0 * up * np.tan(mp - meqn) / (compress * hp)
    ...
    for n in range(0, npoints - 1):
        ...
        (q, cov_x, infodict, mesg, ier) = scio.leastsq(
            fobject,
            q0,
            args=argstup,
            maxfev=10000,
