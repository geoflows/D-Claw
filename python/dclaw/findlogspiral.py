...
def fitnorm(x,x1,y1,x2,y2,alpha1,alpha2):
    ...
    falpha1 = beta1*(np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1)) \
            + np.sin(psi1)*(b*np.sin(theta1)+np.cos(theta1)) - np.sqrt(b**2+1.0))

    falpha2 = beta2*(np.cos(psi2)*(b*np.cos(theta2)-np.sin(theta2)) \
            + np.sin(psi2)*(b*np.sin(theta2)+np.cos(theta2)) - np.sqrt(b**2+1.0))
    ...
    g[2] = -2.*f1x*np.exp(b*theta1)*np.cos(theta1) - 2.*f1y*np.exp(b*theta1)*np.sin(theta1) \
           -2.*f2x*np.exp(b*theta2)*np.cos(theta2) - 2.*f2y*np.exp(b*theta2)*np.sin(theta2) \

    g[3] =  -2.*f1x*a*theta1*np.exp(b*theta1)*np.cos(theta1) \
            -2.*f1y*a*theta1*np.exp(b*theta1)*np.sin(theta1) \
            -2.*f2x*a*theta2*np.exp(b*theta2)*np.cos(theta2) \
            -2.*f2y*a*theta2*np.exp(b*theta2)*np.sin(theta2) \
            +2.*falpha1*(np.cos(psi1)*np.cos(theta1)\
               + np.sin(psi1)*np.sin(theta1)) -(b/np.sqrt(b**2+1.0)) \
            +2.*falpha2*(np.cos(psi2)*np.cos(theta2)\
               + np.sin(psi2)*np.sin(theta2)) -(b/np.sqrt(b**2+1.0)) \

    g[4] = 2.*f1x*(-a*b*np.exp(b*theta1)*np.cos(theta1)+a*np.exp(b*theta1)*np.sin(theta1)) \
            + 2.*f1y*(-a*b*np.exp(b*theta1)*np.sin(theta1)-a*np.exp(b*theta1)*np.cos(theta1)) \
            + 2.*falpha1*(\
            np.cos(psi1)*(-b*np.sin(theta1)-np.cos(theta1)) \
            + np.sin(psi1)*(b*np.cos(theta1)-np.sin(theta1)))

    g[5] = 2.*f2x*(-a*b*np.exp(b*theta2)*np.cos(theta2)+a*np.exp(b*theta2)*np.sin(theta2)) \
            + 2.*f2y*(-a*b*np.exp(b*theta2)*np.sin(theta2)-a*np.exp(b*theta2)*np.cos(theta2)) \
            + 2.*falpha2*(\
...
def spiral_def(x1,y1,x2,y2,alpha1,alpha2,npoints=100):
    ...
    (x,j1,j2) = scio.fmin_tnc(fitnorm,x0,fprime=fitprime,args=(x1,y1,x2,y2,alpha1,alpha2),approx_grad=False,bounds=bounds,maxfun=10000000)
...
def objectf_4x4_scalar(x,x1,y1,x2,y2,alpha1,alpha2):
    ...
    f1  = (x1 -xc)**2.0 + (y1 -yc)**2.0 - a*np.exp(b*theta1)*a*np.exp(b*theta1)
    f2  = (x2 -xc)**2.0 + (y2 -yc)**2.0 - a*np.exp(b*theta2)*a*np.exp(b*theta2)

    falpha1 = -np.sin(psi1)*(b*np.cos(theta1)-np.sin(theta1)) \
              + np.cos(psi1)*(b*np.sin(theta1)+np.cos(theta1))

    falpha2 = -np.sin(psi2)*(b*np.cos(theta2)-np.sin(theta2)) \
              + np.cos(psi2)*(b*np.sin(theta2)+np.cos(theta2))
...
def objectfprime_4x4_scalar(x,x1,y1,x2,y2,alpha1,alpha2,npoints=100):
    ...
    dtanrat1_dyc = -1./(x1-xc)
    dtanrat2_dyc = -1./(x2-xc)
    ...
    df1_dxc = -2.0*(x1 - xc) - a**2.*2.*b*np.exp(2.0*b*theta1)*dtheta1_dxc
    df1_dyc = -2.0*(y1 - yc) - a**2.*2.*b*np.exp(2.0*b*theta1)*dtheta1_dyc
    df1_da  = -2.0*a*np.exp(2.*b*theta1)
    df1_db  = -2.0*a**2*theta1*np.exp(2.*b*theta1)

    df2_dxc = -2.0*(x2 - xc) - a**2.*2.*b*np.exp(2.0*b*theta2)*dtheta2_dxc
    df2_dyc = -2.0*(y2 - yc) - a**2.*2.*b*np.exp(2.0*b*theta2)*dtheta2_dyc
    df2_da  = -2.0*a*np.exp(2.*b*theta2)
    df2_db  = -2.0*a**2*theta2*np.exp(2.*b*theta2)

    dalpha1_dxc = dtheta1_dxc*(-np.sin(psi1)*(b*np.sin(theta1) + np.cos(theta1)) \
                        -np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1)) )
    dalpha1_dyc = dtheta1_dyc*(-np.sin(psi1)*(b*np.sin(theta1) + np.cos(theta1)) \
                        -np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1)) )

    dalpha1_db = np.cos(psi1)*np.sin(theta1) - np.sin(psi1)*np.cos(theta1)
    dalpha1_da = 0.0

    dalpha2_dxc = dtheta1_dxc*(-np.sin(psi1)*(b*np.sin(theta1) + np.cos(theta1)) \
                        -np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1)) )
    dalpha2_dyc = dtheta1_dyc*(-np.sin(psi1)*(b*np.sin(theta1) + np.cos(theta1)) \
                        -np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1)) )
    ...
    g[0] = 2.*f1*df1_dxc + 2.*f2*df2_dxc + 2.*falpha1*dalpha1_dxc + 2.*falpha2*dalpha2_dxc
    g[1] = 2.*f1*df1_dyc + 2.*f2*df2_dyc + 2.*falpha1*dalpha1_dyc + 2.*falpha2*dalpha2_dyc
    g[2] = 2.*f1*df1_da  + 2.*f2*df2_da  + 2.*falpha1*dalpha1_da  + 2.*falpha2*dalpha2_da
    g[3] = 2.*f1*df1_db  + 2.*f2*df2_db  + 2.*falpha1*dalpha1_db  + 2.*falpha2*dalpha2_db
...
def objectf_4x4(x,x1,y1,x2,y2,alpha1,alpha2):
    ...
    f1  = np.sqrt((x1 -xc)**2.0 + (y1 -yc)**2.0) - a*np.exp(b*theta1)#*a*np.exp(b*theta1)
    f2  = np.sqrt((x2 -xc)**2.0 + (y2 -yc)**2.0) - a*np.exp(b*theta2)#*a*np.exp(b*theta2)
...
def objectD_4x4(x,x1,y1,x2,y2,alpha1,alpha2):
    ...
    dalpha_dtheta1 = np.sin(psi1)*(b*np.sin(theta1) + np.cos(theta1)) \
                        +np.cos(psi1)*(b*np.cos(theta1)-np.sin(theta1))
    dalpha_dtheta2 = np.sin(psi2)*(b*np.sin(theta2) + np.cos(theta2)) \
...
def spiral2(x1,y1,x2,y2,alpha1,alpha2,npoints=100,plotspiral=False,verbose=True):
    ...
    if (slopeangle < psi1):
        ...
        raise Exception('INPUT ERROR: scarp angle (alpha1) must be less than average slope angle: %s' % np.rad2deg(slopeangle))

    if (slopeangle > psi2):
        #print 'INPUTERROR: scarp angle (alpha1) must be less than average slope angle'
        raise Exception('INPUT ERROR: toe angle (alpha2) must be greater than average slope angle: %s' % np.rad2deg(slopeangle))
    ...
    if (scarpdiff/toediff<0.3):
        errstr = '****WARNING: scarp and toe angles differ significantly from a log-spiral \n'+\
    ...
    theta1 = np.arctan2(y1-yc,x1-xc) + 2.*np.pi
    theta2 = np.arctan2(y2-yc,x2-xc) + 2.*np.pi

    if (theta1>theta2):
        errstr='ERROR: fit failed*** try decreasing alpha2-alpha1'
        raise Exception(errstr)

    t1 = np.array([b*np.cos(theta1)-np.sin(theta1),b*np.sin(theta1)+np.cos(theta1)])
    t2 = np.array([b*np.cos(theta2)-np.sin(theta2),b*np.sin(theta2)+np.cos(theta2)])
    ...
    erroralpha1 = abs(np.rad2deg(np.arccos(min(1.0,np.dot(t1,a1)/(norm(a1)*norm(t1))))))
    erroralpha2 = abs(np.rad2deg(np.arccos(min(1.0,np.dot(t2,a2)/(norm(a2)*norm(t2))))))
    errorx1  = np.sqrt((x1 - xc- a*np.exp(b*theta1)*np.cos(theta1))**2 \
        +(y1 - yc -a*np.exp(b*theta1)*np.sin(theta1))**2)
    errorx2  = np.sqrt((x2 - xc -a*np.exp(b*theta2)*np.cos(theta2))**2 \
    ...
    if (max((errorx1,errorx2))>1.0):
        errstr= '***WARNING: poor fit: error in enpoints > 1.0 meter\n'+\
        '***try adjusting angles so that following relative differences are closer in magnitude\n'+\
        ('***headscarp: slopeangle-alpha1 = %s degrees\n' %scarpdiff)+\
        ('***toe:       alpha2-slopeangle = %s degrees\n' %toediff)
