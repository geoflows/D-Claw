    ...
    def create(self, fname, gallery_dir=None):
        ...
        for gsec in self.sections:
            ...
            for gitem in gsec.items:
                gfile.write("<p>")
                readme = os.path.join(claw_html_root, gitem.appdir, "README.html")
                plotindex = os.path.join(
                    claw_html_root, gitem.appdir, gitem.plotdir, "_PlotIndex.html"
                )
                gfile.write(
                    '<p><b>$CLAW/%s</b> ... <a href="%s">README</a> ... <a href="%s">Plot Index</a><p>'
...
def make_book():
    ...
    # ----------------------------------------------
    gsec = gallery.new_section(
        "Chapter 3: Characteristics and Riemann"
...
def make_all():
    ...
    gallery_all.sections = (
        gallery_1d.sections
        + gallery_2d.sections
        + gallery_geoclaw.sections
        + gallery_book.sections
    )
