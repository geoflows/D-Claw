...
!     --------------------------------------------------------------------
!     --------------------------------------------------------------------
!	  This version step1fw_dig.f  accumulates the updates
!       into interface fluxes to determine if relimiting is required
!       It then relimits the fluctuations and second-order corrections so that
...
!
!       last modified 1/02/2010, David George.
!     -----------------------------------------------------------------------
...
c==================================Geoclaw===========================================
c     # if relimitall=.true. the fluctuations, amdq and apdq are relimited
     ...
     &          (dmax1(0.d0,fp(1))-dmin1(0.d0,fm(1)))
            ...
            if (outdensity.gt.density) then ! limit interface fluxes
               ...
               ! call subroutine novaccum1(fp,fm,phim,phip,density,dtdx,meqn,meqnlim,tol)
               call novaccum1(fp,fm,density,dtdx(i),
     &                           1,1,tol)
               ...
               summ = amdq(i,1)+apdq(i,1)
               apdq(i,1) = fi(1)-fm(1)
               amdq(i,1) = summ - apdq(i,1)

               sump = apdq(i+1,1) + amdq(i+1,1)
               amdq(i+1,1) = fp(1)-fi(1)
               apdq(i+1,1) = sump - amdq(i+1,1)
